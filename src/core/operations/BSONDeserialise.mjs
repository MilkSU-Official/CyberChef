...
class BSONDeserialise extends Operation {
    ...
    constructor() {
        ...
        this.description = "BSON 是一种计算机数据交换格式，主要用作 MongoDB 数据库的数据存储和网络传输格式。它是用于表示简单数据结构、关联数组（在 MongoDB 中称为对象或文档）以及 MongoDB 特别关注的各种数据类型的二进制形式。'BSON' 这个名称基于 JSON 术语，代表 '二进制 JSON'。<br><br>输入数据应为原始字节格式。";
