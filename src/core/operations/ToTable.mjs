...
class ToTable extends Operation {
    ...
    constructor() {
        ...
        this.name = "To Table";
        this.module = "Default";
        this.description = "数据可以在不同字符上分割，并呈现为 HTML、ASCII 或 Markdown 表格，带有可选的标题行。<br><br>默认支持 CSV（逗号分隔值）文件格式。将单元格分隔符参数更改为 <code>\\t</code> 以支持 TSV（制表符分隔值）或 <code>|</code> 以支持 PSV（管道分隔值）。<br><br>您可以输入任意多个分隔符。每个字符将被视为单独的可能分隔符。";
